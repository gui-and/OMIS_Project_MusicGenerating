...
from keras.layers.convolutional import Convolution1D, Convolution2D, MaxPooling2D, ZeroPadding2D, Deconv2D, UpSampling2D
...
d_input = Input(batch_shape=(batch_size, 784))
...
noise_gen = np.random.uniform(0,1,size=[XT.shape[0],2])
...
def train_for_n(nb_epoch=5000, plt_frq=25, BATCH_SIZE=32):
    ...
    for e in range(nb_epoch):  
        ...
        X0 = np.concatenate((image_batch, generated_images))
...
#        print "Training discriminator"
        ...
        noise_tr = np.random.uniform(0,1,size=[BATCH_SIZE,2])
...
#        print "Training generator"
        g_loss, g_lab = GAN.train_on_batch(noise_tr, y2 )
        losses["g"].append(g_loss)

        # Train vae
#        print "Training VAE"

#        vae_losses = vae.train_on_batch(x_train,x_train)
